...
const BookingFormEditSchema = z.object({
...
type BookingFormEditType = z.infer<typeof BookingFormEditSchema>;

interface BookingFormEditProps {
  booking: Booking;
}

export function BookingFormEdit({ booking }: BookingFormEditProps) {
  const { toast } = useToast();
  const { bookings } = useBookingStore();
  const { setShow } = useContext(SheetContext);

  const formMethods = useForm<BookingFormEditType>({
    resolver: zodResolver(BookingFormEditSchema),
    defaultValues: {
      propertyId: booking.propertyId,
      guestName: booking.guestName ?? 'asdasdasd',
  ...
  const onSubmit = async (data: BookingFormEditType) => {
    try {
      ...
      const isBooked = bookings
        ?.filter(
          (b) => b.property.id === data.propertyId && b.id !== booking.id
        )
        .some((b) => {
      ...
      if (isBooked) {
        ...
        await updateBooking(String(booking.id), newBooking);
